...
Anti-detection strategy:
  ...
  - playwright-stealth v2 patches applied at the context level
...
# playwright-stealth v2.x API — Stealth is a context manager applied per-context
try:
    from playwright_stealth import Stealth
    _STEALTH_V2 = True
except ImportError:
    # Fallback for v1.x installs that still expose stealth_async
...
# Single Stealth instance reused for every context (v2 only)
_stealth = Stealth() if _STEALTH_V2 else None


async def _apply_stealth(page: Page) -> None:
    """Apply stealth patches — handles both v1 and v2 API transparently."""
    if _STEALTH_V2:
        # v2: apply per-page via the async helper
